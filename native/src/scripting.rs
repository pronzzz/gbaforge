...
pub enum ScriptCommand {
    ...
    If1(u32),
    If2(u32),
    Message { text_ptr: u32, type_id: u8 },
    GiveItem { item_id: u16, quantity: u16 },
    TrainerBattle { type_id: u8, trainer_id: u16, unk: u16, ptr_intro: u32, ptr_win: u32 },
...
pub fn disassemble(data: &[u8], start_offset: usize) -> Result<Vec<ScriptCommand>> {
    ...
    loop {
        ...
        match opcode {
            ...
            0x06 => { // if1 (jump if true/result==1)
                 if pc + 4 > data.len() { break; }
                let ptr = u32::from_le_bytes(data[pc..pc+4].try_into()?);
                pc += 4;
                commands.push(ScriptCommand::If1(ptr));
            }
            0x0F => { // msgbox pointer, type
                if pc + 5 > data.len() { break; }
                let ptr = u32::from_le_bytes(data[pc..pc+4].try_into()?);
                let type_id = data[pc+4];
                pc += 5;
                commands.push(ScriptCommand::Message { text_ptr: ptr, type_id });
            }
            0x1A => { // giveitem item_id, quantity
                 if pc + 4 > data.len() { break; }
                 let item_id = u16::from_le_bytes(data[pc..pc+2].try_into()?);
                 let qty = u16::from_le_bytes(data[pc+2..pc+4].try_into()?);
                 pc += 4;
                 commands.push(ScriptCommand::GiveItem { item_id, quantity: qty });
            }
            0x5C => { // trainerbattle type, id, unk, ptr_intro, ptr_win (simplified)
                // structure varies by type, assuming type 0 (standard)
                // byte type, hword id, hword unk, word intro, word win
                if pc + 13 > data.len() { break; }
                let type_id = data[pc];
                ...
                pc += 13;
                commands.push(ScriptCommand::TrainerBattle { type_id, trainer_id, unk, ptr_intro, ptr_win });
            }
            _ => {
